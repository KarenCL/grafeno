...
try:
    print("(_____test.py): dentro del try")
    import argcomplete
    import glob
    from os.path import dirname, basename
    def config_completer(prefix, **kwargs):
        print(" (_____test.py) - config_completer")
...
if args.config_file:
    try:
        print("(_____test.py): se ha usado la opcion -c")
        print("(_____test.py): accediendo a %r.yaml" % args.config_file)
        config_file = open('configs/'+args.config_file+'.yaml')
    except FileNotFoundError:
        print("(_____test.py): archivo de configuracion .yaml no encontrado")
        config_file = None
    if not config_file:
        print("(_____test.py): accediendo al file_config sin extension")
...
#si no se usan los ficheros de configuracion, se opera con los transformadores
    ...
    if args.transformers:
        ...
        print("(_____test.py): args.transformers-> %r" % args.transformers)
    if args.linearizers:
        config['linearizers'] = args.linearizers
        print("(_____test.py): args.linearizers-> %r" % args.linearizers)


config['text'] = text

result = pipeline.run(config)
print("(_____test.py): resultado del pipeline.run\n")
